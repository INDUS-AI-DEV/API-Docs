...
<section id="stt-basic" style={styles.sectionCard}>
        ...
        <p style={styles.sectionDescription}>
          Transcribe audio files using the HTTP API. This is suitable for complete file uploads where immediate streaming feedback is not required.
        </p>
        <div style={styles.codeExample}>
          <CopyableCode language="python">{`from induslabs import Client
...
print(f"Request ID: {result.request_id}")`}</CopyableCode>
      ...
      <section id="stt-streaming" style={styles.sectionCard}>
        <h2 style={styles.sectionTitle}>Real-time Streaming (WebSocket)</h2>
        <p style={styles.sectionDescription}>
          Use WebSocket streaming to receive transcription segments in real-time as they are processed. This is ideal for long files or low-latency requirements.
        </p>
        <div style={styles.codeExample}>
...
# 1. Define a callback to handle segments as they arrive
...
result = client.stt.transcribe_stream(
    file="audio.wav",
    on_segment=on_segment
)

# 3. Access final results and metrics
print(f"\n✅ Complete transcription: {result.final_text}")
...
if result.has_error:
    print(f"Error: {result.error}")`}</CopyableCode>
        </div>
        <div style={styles.callout}>
          <strong>Note:</strong> The <code>on_segment</code> callback is triggered every time a new stable text segment is generated by the engine.
        </div>
      </section>
      ...
      <section id="stt-file-objects" style={styles.sectionCard}>
        ...
        <p style={styles.sectionDescription}>
          Transcribe audio from in-memory file objects (like <code>BytesIO</code>) or open file handles without saving to disk first.
        </p>
        <div style={styles.codeExample}>
...
result = client.stt.transcribe(file=audio_buffer)`}</CopyableCode>
      ...
      <section id="async-api" style={styles.sectionCard}>
        ...
        <p style={styles.sectionDescription}>
          Use async methods for concurrent requests and non-blocking operations. Both HTTP and WebSocket streaming are supported asynchronously.
        </p>
        <div style={styles.codeExample}>
...
async def main():
    async with Client(api_key="your_api_key") as client:
        ...
        # --- Example 1: Async HTTP Transcription ---
        print("Starting HTTP transcription...")
        result = await client.stt.transcribe_async("audio.wav")
        print(f"HTTP Result: {result.text}")

        # --- Example 2: Async WebSocket Streaming ---
        ...
        stream_result = await client.stt.transcribe_stream_async(
            "audio.wav",
            on_segment=on_segment
        )
        
        print(f"Final Stream Text: {stream_result.final_text}")
...
asyncio.run(main())`}</CopyableCode>
      ...
      <section id="concurrent-requests" style={styles.sectionCard}>
        ...
        <p style={styles.sectionDescription}>
          Process multiple requests in parallel for better throughput using <code>asyncio.gather</code>.
        </p>
        <div style={styles.codeExample}>
          <CopyableCode language="python">{`import asyncio
from induslabs import Client
...
response.get_audio_data()            # Get raw bytes`}</CopyableCode>
        ...
        <h3>STTResponse (HTTP)</h3>
        <div style={styles.codeExample}>
          <CopyableCode language="python">{`result = client.stt.transcribe("audio.wav")

# Properties
result.text                          # str: Transcribed text
result.language_detected             # str: Detected language (e.g., 'hi')
result.audio_duration_seconds        # float: Duration of source audio
result.processing_time_seconds       # float: Time taken to process
result.first_token_time_seconds      # float: Latency to first token
result.credits_used                  # int: API credits consumed
result.request_id                    # str: Unique request identifier`}</CopyableCode>
        </div>

        <h3>STTStreamResponse (WebSocket)</h3>
        <div style={styles.codeExample}>
          <CopyableCode language="python">{`result = client.stt.transcribe_stream("audio.wav", on_segment=callback)

# Properties
result.final_text                    # str: Complete combined text
...
result.metrics.rtf                   # float: Real-time Factor
result.metrics.transcription_time    # float: Time spent transcribing
result.metrics.total_time            # float: Total session time`}</CopyableCode>
        </div>
